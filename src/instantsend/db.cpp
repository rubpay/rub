...
CInstantSendDb::CInstantSendDb(const util::DbWrapperParams& db_params) :
    db{util::MakeDbWrapper({db_params.path / "llmq" / "isdb", /*cache_size=*/32 << 20, db_params.memory, db_params.wipe})}
{
    Upgrade({db_params.path / "llmq" / "isdb", /*cache_size=*/32 << 20, db_params.memory, /*wipe=*/true});
}
